...
namespace ECX.CD.DA
{
    public class PUN : SQLHelper
    {
        ...
        public DataTable GetPUNForExtendExpiryApproval()
		{
            ...
            @"SELECT   tblExtendPUNExpiryDate.Id, tblPickUpNotice.PUNId, tblPickUpNotice.WarehouseId, 
            clWarehouses.[Description] AS WarehouseName, 
            tblPickUpNoticeWarehouseReciept.WarehouseRecieptId AS WarehouseRecieptIds, 
            ...
            tblExtendPUNExpiryDate ON tblPickUpNotice.Id = tblExtendPUNExpiryDate.PUNId INNER JOIN 
            tblExtendExpiryDateStatus ON tblExtendPUNExpiryDate.Status = tblExtendExpiryDateStatus.Id 
            INNER JOIN clWarehouses on clWarehouses.ID=tblPickUpNotice.WarehouseId 
            inner join tblPickUpNoticeWarehouseReciept on tblPickUpNoticeWarehouseReciept.PickupNoticeId=tblPickUpNotice.Id 
            WHERE   (tblExtendExpiryDateStatus.Name = 'New')";
...
//if (new GINDetail.GINDetail().ExtendPUN(GetExpiredPUNExtension(extendExpiryDateIds), userId, DateTime.Now))
            if (UpdatePUNExpiryDate_OnWarehouseDB(extendExpiryDateIds,userId))
        ...
        /// <summary>
        /// tg new method to update PUN expiry date extension on warehouse DB
        /// </summary>
        /// <param name="extendExpiryDateIds"></param>
        /// <returns></returns>
        public bool UpdatePUNExpiryDate_OnWarehouseDB(List<Guid> extendExpiryDateIds, Guid userId)
        {
            List<SqlCommand> commands = new List<SqlCommand>();
            SqlCommand command;
            BE.PUN.ExtendPUNExpiryDateRow pUNExtension;
             
            foreach (Guid extendId in extendExpiryDateIds)
            {
                pUNExtension = GetPUNExtension(extendId);
                command = new SqlCommand();
                command.Parameters.Add("@Id", SqlDbType.UniqueIdentifier).Value = pUNExtension.PUNId;
                command.Parameters.Add("@NumberOfDays", SqlDbType.Int).Value = pUNExtension.NumberOfDays;
                command.Parameters.Add("@LastModifiedBy", SqlDbType.UniqueIdentifier).Value = userId;
                command.Parameters.Add("@LastModifiedTimestamp", SqlDbType.DateTime).Value = DateTime.Now;

                command.CommandType = CommandType.Text;
                command.CommandText = "UPDATE tblPickupNotices SET ExpirationDate=DateAdd(Day, @NumberOfDays, ExpirationDate) , LastModifiedBy=@LastModifiedBy , LastModifiedTimestamp=@LastModifiedTimestamp " +
                                       "WHERE ID=@Id";
                commands.Add(command);
            }

            return ExecuteTransaction_InWarehouseDB(commands);
        }
        public BE.PUN.ExtendPUNExpiryDateRow GetPUNExtension(Guid pUNExtensionId)
        {
            BE.PUN.ExtendPUNExpiryDateDataTable ret = new ECX.CD.BE.PUN.ExtendPUNExpiryDateDataTable();
            SqlCommand command = new SqlCommand();

            command.CommandText = "Select * From [tblExtendPUNExpiryDate] Where [Id]=@Id ";

            command.Parameters.Add("Id", SqlDbType.UniqueIdentifier).Value = pUNExtensionId;
        ...
        public List<SqlCommand> DeductMoistureLoss(Guid extendPUNExpiryId)
        {
            ...
            foreach (DataRow item in items.Rows)
            {
                ...
                command.Parameters.Add("WarehouseRecieptId", SqlDbType.Int).Value = Convert.ToInt32(item["WarehouseRecieptId"]);
                command.Parameters.Add("PickupNoticeId", SqlDbType.UniqueIdentifier).Value = new Guid(item["PickupNoticeId"].ToString());
		...
		public float TotalPUNQuantity(Guid WarehouseId, Guid CommodityGradeId, int productionYear)
        {
            ...
            command.CommandText =
				...
				"WHERE   (tblPickUpNotice.CommodityGradeId = @CommodityGradeId) AND " +
                        "(tblPickUpNotice.WarehouseId = @WarehouseId) And " +
                        "(ProductionYear = @ProductionYear) And (tblPUNStatus.Name <> 'Cancelled')";
            ...
            string tradeDateFrom, string tradeDateTo)
        {
            ...
            whereClause += (tradeDateFrom == "") ? "" : "tblWarehouseReciept.TradeDate >= @TradeDateFrom And ";
            whereClause += (tradeDateTo == "") ? "" : "tblWarehouseReciept.TradeDate <= @TradeDateTo And ";
            ...
            if (tradeDateFrom == "" && tradeDateTo == "")
            {
                command.CommandText =
                    "SELECT tblPickUpNotice.Id, tblPickUpNotice.PUNId, tblPickUpNotice.WarehouseId, " +
                        ...
                        "tblPickUpNotice.ClientId As ClientGuid, '' As ClientId, '' As ClientName, " +
                        "tblPickUpNotice.CommodityGradeId, '' As Symbol, tblPickUpNotice.CreatedTimestamp, " +
                        "tblPickUpNoticeWarehouseReciept.Quantity, '' As TradeDates " +
                    "FROM   tblPickUpNotice INNER JOIN " +
                        "tblPUNStatus ON tblPickUpNotice.PUNStatusId = tblPUNStatus.Id INNER JOIN " +
                        "tblPickUpNoticeWarehouseReciept ON tblPickUpNotice.Id = tblPickUpNoticeWarehouseReciept.PickupNoticeId " +
                    whereClause + " " +
            ...
            else
            {
                command.CommandText =
                    ...
                    "FROM   tblPickUpNotice INNER JOIN " +
                        "tblPUNStatus ON tblPickUpNotice.PUNStatusId = tblPUNStatus.Id  INNER JOIN " +
                        "tblPickUpNoticeWarehouseReciept ON tblPickUpNotice.Id = tblPickUpNoticeWarehouseReciept.PickupNoticeId INNER JOIN " +
                        "tblWarehouseReciept ON tblPickUpNoticeWarehouseReciept.WarehouseRecieptId = tblWarehouseReciept.WarehouseRecieptId " +
                    whereClause + " " +
                    "Order By tblPickUpNotice.PUNId";
            }
            ...
            if (!string.IsNullOrEmpty(tradeDateFrom)) command.Parameters.Add("TradeDateFrom", SqlDbType.DateTime).Value = Convert.ToDateTime(tradeDateFrom);
            if (!string.IsNullOrEmpty(tradeDateTo)) command.Parameters.Add("TradeDateTo", SqlDbType.DateTime).Value = Convert.ToDateTime(tradeDateTo);
            ...
            string tradeDateFrom, string tradeDateTo)
        {
            ...
            command.CommandText =
                "SELECT tblPickUpNotice.Id, tblPickUpNotice.PUNId, tblPickUpNotice.WarehouseId, " +
                    "'' As WarehouseName, '' As WarehouseRecieptIds, tblPickUpNotice.ExpirationDate, " +
                    "tblPickUpNotice.AgentName, tblPickUpNotice.AgentTel, tblPickUpNotice.NIDType As NIDTypeId, " +
                    "tblNIDType.Name As NIDTypeName, tblPickUpNotice.NIDNumber, tblPUNStatus.Name As PUNStatusName, " +
                    "ClientId As ClientGuid, '' As ClientId, '' As ClientName, CommodityGradeId, '' As Symbol, CreatedTimestamp " +
                "FROM   tblPickUpNotice INNER JOIN " +
                    "tblPUNStatus ON tblPickUpNotice.PUNStatusId = tblPUNStatus.Id Inner Join " +
                    "tblNIDType ON tblNIDType.Id = tblPickUpNotice.NIDType " +
                whereClause + " " +
                "Order By tblPickUpNotice.PUNId";
        ...
        public BE.PUN.PickUpNoticeWarehouseRecieptDataTable GetPickupNoticeWarehouseReciepts(Guid pickupNoticeId)
        {
            ...
            command.CommandText =
                "SELECT  Cast(1 AS bit) As Selected, tblPickUpNoticeWarehouseReciept.WarehouseRecieptId, " +
                        ...
                        "Cast(0 As float) As Shortfall, " +
                        "tblPickUpNoticeWarehouseReciept.DNReceivedDateTime, '' As TradeDate, " +
                        "(tblWarehouseReciept.OriginalQuantity - tblPickUpNoticeWarehouseReciept.Quantity) As QuantityRemaining, " +
                        "tblWarehouseReciept.OriginalQuantity " +
                "FROM	tblPickUpNoticeWarehouseReciept INNER JOIN " +
                        "tblWarehouseReciept ON tblPickUpNoticeWarehouseReciept.WarehouseRecieptId = tblWarehouseReciept.WarehouseRecieptId " +
                "WHERE   (tblPickUpNoticeWarehouseReciept.PickupNoticeId = @PickupNoticeId)";
            ...
            string WashingMillingStation, string TrailerPlateNumber, string PlateNumber, string SellerName, string Shade, bool isTracable, string GRNNumber)
        {
            ...
            command.Parameters.Add("@ConsignmentType", SqlDbType.VarChar).Value = ConsignmentType;
            command.Parameters.Add("@SustainableCertification", SqlDbType.Text).Value = SustainableCertification;
            command.Parameters.Add("@Woreda", SqlDbType.VarChar).Value = Woreda;
            command.Parameters.Add("@RawValue", SqlDbType.Decimal).Value = RawValue;
            command.Parameters.Add("@Cupvalue", SqlDbType.Decimal).Value = Cupvalue;
            command.Parameters.Add("@TotalValue", SqlDbType.Decimal).Value = TotalValue;
            command.Parameters.Add("@WashingMillingStation", SqlDbType.VarChar).Value = WashingMillingStation;
            command.Parameters.Add("@TrailerPlateNumber", SqlDbType.VarChar).Value = TrailerPlateNumber;
            command.Parameters.Add("@PlateNumber", SqlDbType.VarChar).Value = PlateNumber;
            command.Parameters.Add("@SellerName", SqlDbType.VarChar).Value = SellerName;
            command.Parameters.Add("@Shade", SqlDbType.NVarChar).Value = Shade;
            command.Parameters.Add("@IsTraceable", SqlDbType.Bit).Value = isTracable;
            command.Parameters.Add("@GRNNumber", SqlDbType.NVarChar).Value = GRNNumber;
            ...
            //bool isSeasame = false;
            //if (CC.CommodityUniqueIdentifier.ToString().ToLower() != "d97fd8c1-8916-4e3d-89e2-bd50d556a32f".ToLower())
            //{
                
            //    try
            //    {
            //        lossAmount = (double)ecxlookup.GetCommodityGrade(Common.EnglishGuid, CommodityGradeId).MLA;
            //    }
            //    catch
            //    {
            //        lossAmount = 0;
            ...
            foreach (BE.PUN.PickUpNoticeWarehouseRecieptRow row in punWHRs)
            {
                ...
                //if (isSeasame)
                //{
                //    double netWeightLoss = row.Weight - (lossAmount * row.Weight);
                //    commands.Add(SavePUNWR(
                //    row.WarehouseRecieptId, row.PickupNoticeId, row.Quantity, netWeightLoss,
                //    row.DNReceivedDateTime, row.CreatedBy, row.Weight));
                //}
                //else
                //{
        ...
        public SqlCommand SavePUNWR(Int32 WarehouseRecieptId, Guid PickupNoticeId,
            Double Quantity, Double Weight, DateTime DNReceivedDateTime, Guid UserId, Double WeightBeforeMoisture)
        {
            SqlCommand command = new SqlCommand();
        ...
        public DataTable GetPUNForEditApproval()
        {
            ...
            command.CommandText =
                "SELECT  tblPUNEdit.Id, tblPickUpNotice.PUNId, tblPickUpNotice.Id As PickUpNoticeId, tblPickUpNotice.WarehouseId, '' AS WarehouseName, " +
                        ...
                        "Cast(tblPickUpNotice.NIDType As Varchar(50)) As NIDType, " +
                        "Cast(tblPUNEdit.NIDType As Varchar(50)) As NewNIDType, " +
            ...
            ret = ExecuteDT(command);
            foreach (DataRow row in ret.Rows)
            {
                row["NIDType"] = lu.GetLookupName("tblNIDType", Convert.ToByte(row["NIDType"]));
                row["NewNIDType"] = lu.GetLookupName("tblNIDType", Convert.ToByte(row["NewNIDType"]));
            }
        ...
        public bool ApprovePUNEdit(Guid userId, List<Guid> pUNEditIds)
        {
            ...
            SendPUNEditToWH_WithQuery(userId, pUNEditIds);
            //SendPUNEditToWH(pUNEditIds);
            return ExecuteTransaction(commands);
        }

        public void SendPUNEditToWH(List<Guid> pUNEditIds)
        {
            BE.PUN.PUNEditRow pUNEdit;
            foreach (Guid pUNEditId in pUNEditIds)
            {
                pUNEdit = GetPUNEdit(pUNEditId);
               bool ret= new GINDetail.GINDetail().UpdatePickupAgent(pUNEdit.PUNId.ToString(), pUNEdit.AgentName, pUNEdit.NIDType, pUNEdit.NIDNumber, pUNEdit.AgentTel);
            }
        }

        /// <summary>
        /// tg new method to update PUN agent detail on warehouse database
        /// </summary>
        /// <param name="pUNEditId"></param>
        /// <returns></returns>
        public bool SendPUNEditToWH_WithQuery(Guid userId, List<Guid> pUNEditIds)
        {
            List<SqlCommand> commands = new List<SqlCommand>();
            SqlCommand command;
            BE.PUN.PUNEditRow pUNEdit;
            foreach (Guid pUNEditId in pUNEditIds)
            {
                ...
                command = new SqlCommand();
                command.Parameters.Add("Id", SqlDbType.UniqueIdentifier).Value = pUNEdit.PUNId;
                command.Parameters.Add("AgentName", SqlDbType.NVarChar).Value = pUNEdit.AgentName;
                command.Parameters.Add("IDTypeID", SqlDbType.Int).Value = pUNEdit.NIDType;
                command.Parameters.Add("IDType", SqlDbType.VarChar).Value = new ECX.CD.DA.Lookup().GetLookupName("tblNIDType",Convert.ToByte(pUNEdit.NIDType));
                command.Parameters.Add("AgentIDNumber", SqlDbType.NVarChar).Value = pUNEdit.NIDNumber;
                command.Parameters.Add("AgentTel", SqlDbType.VarChar).Value = pUNEdit.AgentTel;
                command.Parameters.Add("LastModifiedBy", SqlDbType.UniqueIdentifier).Value = userId;
                command.Parameters.Add("LastModifiedTimestamp", SqlDbType.DateTime).Value = DateTime.Now;

                command.CommandType = CommandType.Text;
                command.CommandText = "UPDATE tblPickupNotices SET AgentName=@AgentName , IDTypeID=@IDTypeID , IDType=@IDType , AgentIDNumber=@AgentIDNumber , " +
                                       "AgentTel=@AgentTel , LastModifiedBy=@LastModifiedBy , LastModifiedTimestamp=@LastModifiedTimestamp " +
                                       "WHERE ID=@Id";
                commands.Add(command);
            }
        ...
        public BE.PUN.PUNEditRow GetPUNEdit(Guid pUNEditId)
        {
            BE.PUN.PUNEditDataTable ret = new ECX.CD.BE.PUN.PUNEditDataTable();
            SqlCommand command = new SqlCommand();

            command.CommandText = "Select * From [tblPUNEdit] Where [Id]=@Id ";
